...
class App extends Component {
  ...
  standardButtonHandler = () => {                         // For standard game
  ...
  expertButtonHandler = () => {                           // For expert game
  ...
  validation = () => { 
    ...
    const highScores = (this.state.level === "standard") ? this.state.highscores.standard : this.state.highscores.expert 
    ...
    } else if(number > Number(guess)){    // If the random number is greater than the guess
      this.setState({
    ...
    } else if (number < Number(guess) ){    // If the random number is less than the guess
      this.setState({
        counter: this.state.counter + 1,
        message: 'Guess Lower'
      })
  ...
  resetPage = () => {   // Resets page to normal when function is called
    this.setState({
      number: null,
      guess: null,
      message: null,
